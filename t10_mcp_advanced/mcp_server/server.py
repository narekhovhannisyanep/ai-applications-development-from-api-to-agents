...
def _validate_accept_header(accept_header: str | None) -> bool:
    ...
    accept_types = [t.lower().strip() for t in accept_header.split(",")]
...
async def _create_sse_stream(messages: list):
    ...
    for m in messages:
        event_data = f"data: {json.dumps(m.dict(exclude_none=True))}\n\n"
...
async def handle_mcp_request(
    ...
    if request.method == "initialize":
        ...
        # Handle notifications that do not need responses
